...
function(vclib_build_shader)
    ...
    if(APPLE)
        list(
            APPEND BGFX_VF_PROFILES
            metal22-11 # metal
        )
        list(
            APPEND BGFX_COMPUTE_PROFILES
            metal22-11 # metal
        )
    endif()
    if(WIN32)
    ...
    if("${TYPE}" STREQUAL "COMPUTE")
        ...
        if(ARG_VARYING_DEF)
            ...
            set(VARYING_DEF_ARGUMENT
                VARYING_DEF
                ${ABSOLUTE_DIR_PATH}/varying.def.sc
            )
        endif()
    ...
    foreach(SHADER ${ARGV})
        ...
        if(IS_IN_BIN_DIR EQUAL 0)
            file(
                RELATIVE_PATH
                SHADER_REL
                "${TARGET_BIN_DIR}/shaders/vclib/bgfx"
                ${SHADER}
            )
            get_filename_component(DIR_PATH ${SHADER_REL} DIRECTORY)
            set(VARYING_DEF_FILE
                "${VCLIB_RENDER_SHADER_DIR}/vclib/bgfx/${DIR_PATH}/varying.def.sc"
            )
        else()
            file(
                RELATIVE_PATH
                SHADER_REL
                "${VCLIB_RENDER_SHADER_DIR}/../shaders/vclib/bgfx"
                ${SHADER}
            )
            get_filename_component(DIR_PATH ${SHADER_REL} DIRECTORY)
            set(VARYING_DEF_FILE "")
...
function(_vclib_embedded_program_validate_args ENUM_NAME TYPE FILES)
    if(NOT ENUM_NAME)
        message(
            FATAL_ERROR
            "vclib_add_embedded_program: missing ENUM_NAME argument"
        )
    endif()
    ...
    if(NOT FILES)
        message(
            FATAL_ERROR
            "vclib_add_embedded_program: missing FILES argument"
        )
    endif()
endfunction()

function(
    _vclib_embedded_program_resolve_type
    TYPE
    OUT_PROGRAM_TYPE
    OUT_PROGRAM_TYPE_LC
)
    ...
    elseif(TYPE_UC STREQUAL "COMPUTE")
        ...
        message(
            FATAL_ERROR
            "vclib_add_embedded_program: TYPE must be VERT_FRAG or COMPUTE"
        )
    endif()
...
function(
    ...
    set(GENERATED_HEADER_DIR
        "${TARGET_BIN_DIR}/include/vclib/bgfx/programs/embedded_${PROGRAM_TYPE_LC}_programs"
    )
    set(GENERATED_SOURCE_DIR
        "${TARGET_BIN_DIR}/src/vclib/bgfx/programs/embedded_${PROGRAM_TYPE_LC}_programs"
    )
...
function(
    ...
    if(PROGRAM_TYPE STREQUAL "VF")
        ...
        if(NUM_FILES GREATER 2)
            message(
                FATAL_ERROR
                "vclib_add_embedded_program: VERT_FRAG programs require exactly 2 files (vertex and fragment)"
            )
        endif()
...
function(
    ...
    set(NEW_INCLUDE
        "#include \"embedded_${PROGRAM_TYPE_LC}_programs/${ENUM_NAME_LC}.h\""
    )
...
function(vclib_add_embedded_program)
    ...
    set(HEADER_TEMPLATE
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/templates/embedded_${PROGRAM_TYPE_LC}_programs/header.h.in"
    )
    ...
    set(SOURCE_TEMPLATE
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/templates/embedded_${PROGRAM_TYPE_LC}_programs/source.cpp.in"
    )
