...
int main()
{
    ...
    vcl::savePly(m, VCL_TEST_RESULTS_PATH "/tetrahedron.ply", info);

    m = vcl::createIcosahedron<vcl::TriMesh>(true);
    vcl::savePly(m, VCL_TEST_RESULTS_PATH "/icosahedron.ply", info);

    m = vcl::createHexahedron<vcl::TriMesh>();
    vcl::savePly(m, VCL_TEST_RESULTS_PATH "/hexahedron.ply", info);

    vcl::ConsoleLogger log;
    log.setPrintTimer(true);
    log.startTimer();
    m = vcl::createDodecahedron<vcl::TriMesh>(log);
    vcl::savePly(m, VCL_TEST_RESULTS_PATH "/dodecahedron.ply", info);

    vcl::PolyMesh pm = vcl::createHexahedron<vcl::PolyMesh>();
    vcl::savePly(pm, VCL_TEST_RESULTS_PATH "/hexahedron_poly.ply", info);
    ...
    vcl::savePly(m, VCL_TEST_RESULTS_PATH "/sphere_ico.ply");
