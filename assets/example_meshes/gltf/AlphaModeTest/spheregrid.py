...
def create_sphere_with_material(x, y, base_color_alpha, alpha_mode):
    ...
    base_color[3] = base_color_alpha  # Set alpha in base color growing with x
...
for i in range(len(alpha_modes)):
    ...
    for y in range(grid_size):
        for x in range(grid_size):
            base_color_alpha = x / (grid_size - 1)
            mesh, material = create_sphere_with_material(x, y, base_color_alpha, alpha_mode)
