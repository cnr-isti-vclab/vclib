...
using WindowHeadless = vcl::RenderApp<vcl::HeadlessManager, vcl::Canvas, HelloTriangleDrawer>;
...
TEST_CASE("Headless Triangle Rendering")
{
    ...
    std::string groundTruthFilename = std::string(VCLIB_GROUND_TRUTH_PATH) + "/001-hello-triangle-headless_gt.png";
    ...
    if (!std::filesystem::exists(groundTruthFilename)) {
        ...
        FAIL("Ground truth image not found. Saved the current render to: " + groundTruthFilename + 
             "\nPlease verify the image visually. If it is correct, commit it and re-run the test.");
    }
