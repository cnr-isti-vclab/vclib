...
/**
 * @brief Returns a const reference of the %EL_UC% at the i-th position in the %EL_UC% Container of the
 * Mesh, which will be the %EL_UC% having index = i.
 *
 * This function does not perform any sanity check: if i is less than %EL_C%ContainerSize(), this
 * function will return a valid %EL_UC% reference (note that the %EL_UC% may have been flagged as
 * deleted).
 *
 * @param[in] i: the index of the %EL_UC% that will be returned.
 */
template<%EL_UC%Concept T>
const typename %EL_UC%Container<T>::%EL_UC%Type& %EL_UC%Container<T>::%EL_C%(uint i) const
{
	return Base::element(i);
}

/**
 * @brief Returns a reference of the %EL_UC% at the i-th position in the %EL_UC% Container of the Mesh,
 * which will be the %EL_UC% having index = i.
 *
 * This function does not perform any sanity check: if i is less than %EL_C%ContainerSize(), this
 * function will return a valid %EL_UC% reference (note that the %EL_UC% may have been flagged as
 * deleted).
 ...
 */
template<%EL_UC%Concept T>
typename %EL_UC%Container<T>::%EL_UC%Type& %EL_UC%Container<T>::%EL_C%(uint i)
{
	return Base::element(i);
}

/**
 * @brief Returns the number of **non-deleted** %EL_UC%s contained in the %EL_UC% container of the Mesh.
 *
 * If %EL_C%Number() != %EL_C%ContainerSize(), it means that there are some %EL_UC%s that are
 * flagged as deleted.
 *
...
/**
 * @brief Returns the number of %EL_UC%s (also deleted) contained in the %EL_UC% container of the Mesh.
 *
 * If %EL_C%Number() != %EL_C%ContainerSize(), it means that there are some %EL_UC%s that are
 * flagged as deleted.
 *
...
/**
 * @brief Returns the number of deleted %EL_UC%s in the %EL_UC% container, that is %EL_C%ContainerSize() -
 * %EL_C%Number().
 *
...
/**
 * @brief Add an arbitrary number of n %EL_UC%s, returning the id of the first added %EL_UC%.
 *
 * This means that, if you want to add 5 %EL_UC%s and this member function returns 4, the added
 * %EL_UC%s will have id from 4 to id 8 included.
 *
 * If the call of this function will cause a reallocation of the %EL_UC%Container, the function
 * will automatically take care of updating all the %EL_UC% pointers contained in the Mesh.
 *
...
/**
 * @brief Reserve a number of %EL_UC%s in the container of %EL_UC%s. This is useful when you know
 * (or you have an idea) of how much %EL_UC%s are going to add into a newly of existing mesh.
 * Calling this function before any add%EL_UC%() call will avoid unuseful reallocations of the
...
/**
 * @brief Compacts the %EL_UC%Container, removing all the %EL_UC%s marked as deleted. %EL_UC%s indices
 * will change accordingly. The function will automatically take care of updating all the %EL_UC%
 ...
 *
 * This member function does not perform any reallocation of the %EL_UC%s: the deleted %EL_UC%s
 * will stay in the %EL_UC% Container, but will be marked as deleted.
 *
 * Deleted %EL_UC%s are automatically jumped by the iterators provided by the %EL_UC% Container.
 *
...
/**
 * @brief Marks as deleted the given %EL_UC%, before asserting that the %EL_UC% belongs to this container.
 *
 * This member function does not perform any reallocation of the %EL_UC%s: the deleted %EL_UC%s
 * will stay in the %EL_UC% Container, but will be marked as deleted.
 *
 ...
 * would be compact, that is the number of non-deleted elements before the %EL_UC% with the given
 * index.
 *
 ...
 * @return The index that the %EL_UC% with index i would have if this container would be compact.
 */
template<%EL_UC%Concept T>
 ...
 *
 * @param[in] jumpDeleted (def: true): boolean that tells if the iterator should jump deleted %EL_UC%s.
 * @return An iterator the the first %EL_UC% of the container.
 */
template<%EL_UC%Concept T>
typename %EL_UC%Container<T>::%EL_UC%Iterator %EL_UC%Container<T>::%EL_C%Begin(bool jumpDeleted)
{
 ...
 */
template<%EL_UC%Concept T>
typename %EL_UC%Container<T>::Const%EL_UC%Iterator %EL_UC%Container<T>::%EL_C%Begin(bool jumpDeleted) const
{
	return Base::elementBegin(jumpDeleted);
}
 ...
 */
template<%EL_UC%Concept T>
typename %EL_UC%Container<T>::Const%EL_UC%Iterator %EL_UC%Container<T>::%EL_C%End() const
{
	return Base::elementEnd();
}

/**
 * @brief Returns a small view object that allows to iterate over the %EL_UC%s of the containers,
 * providing two member functions begin() and end().
 *
 * This member function is very useful when you want to iterate over the %EL_UC%s using the C++ foreach
 * syntax:
 *
 * @code{.cpp}
 ...
 * @endcode
 *
 * The iterator used to iterate over %EL_UC%s is automatically initialized to jump deleted %EL_UC%s of the
...
/**
 * @brief Enables all the optional components associated to the %EL_UC% type contained in the
 * %EL_UC%Container.
 */
template<%EL_UC%Concept T>
...
/**
 * @brief Disables all the optional components associated to the %EL_UC% type contained in the
 * %EL_UC%Container.
 */
template<%EL_UC%Concept T>
