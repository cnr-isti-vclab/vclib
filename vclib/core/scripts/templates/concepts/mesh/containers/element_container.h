    ...
    namespace vcl
{
    ...
    /**
     * @brief The Has%EL_UC%Container concept is satisfied only if a container
     * class provides the types and member functions specified in this concept.
     * These types and member functions allow to access to a @ref
     * vcl::mesh::%EL_UC%Container container of a given mesh.
     *
     * @ingroup containers_concepts
     */
    template<typename T>
        concept Has % EL_UC % Container =
        requires (T o, const T& co, typename T:: % EL_UC % Type * e) {
            // clang-format off
     ...
     *
     * In the following example, a MyMesh type can be instantiated only if one
     * of its template Args is a %EL_UC%Container:
     * @code{.cpp}
    ...
    template<typename... Args>
        concept Has % EL_UC %
        s = (vcl::mesh::Has % EL_UC % Container<Args> || ...);
