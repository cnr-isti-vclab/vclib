            ...
            if (0 == strncmp(s, "include", 7) && stb_include_isspace(s[7])) {
                ...
                if (*s == '"') {
                    ...
                    if (*t == '"') {
                        ...
                        while (*s != '\r' && *s != '\n' && *s != 0)
                            ++s;
                        // s points to the newline, so s-start is everything
                        // except the newline
...
static char* stb_include_append(
    char*   str,
    size_t* curlen,
    char*   addstr,
    size_t  addlen)
    ...
    char  error[256])
{
    ...
    for (i = 0; i < num; ++i) {
        text = stb_include_append(
            text, &textlen, str + last, inc_list[i].offset - last);
// write out line directive for the include
#ifndef STB_INCLUDE_LINE_NONE
#ifdef STB_INCLUDE_LINE_GLSL
        if (textlen != 0) // GLSL #version must appear first, so don't put a
                          // #line at the top
#endif
        {
            strcpy(temp, "#line ");
        ...
        if (inc_list[i].filename == 0) {
            if (inject != 0)
                text =
                    stb_include_append(text, &textlen, inject, strlen(inject));
        }
        else {
            char* inc;
...
// next
    ...
    text = stb_include_append(
        text, &textlen, str + last, source_len - last + 1); // append '\0'
    stb_include_free_includes(inc_list, num);
    return text;
}
    ...
    char   error[256])
{
    ...
    result =
        stb_include_string(text, inject, path_to_includes, filename, error);
    free(text);
    return result;
}
