    ...
    int)
{
    ...
    return umfpack_zi_get_determinant(
        &mx_real, 0, Ex, NumericHandle, User_Info);
}
...
public:
    ...
    bool _solve_impl(const MatrixBase<BDerived>& b, MatrixBase<XDerived>& x)
        const;
...
void UmfPackLU<MatrixType>::extractData() const
{
    if (m_extractedDataAreDirty) {
        ...
        umfpack_get_lunz(
            &lnz, &unz, &rows, &cols, &nz_udiag, m_numeric, Scalar());
