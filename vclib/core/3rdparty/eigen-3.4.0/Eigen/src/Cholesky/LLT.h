...
struct llt_inplace<Scalar, Upper>
{
    ...
    {
        Transpose<MatrixType> matt(mat);
        return llt_inplace<Scalar, Lower>::rankUpdate(
...
template<typename _MatrixType, int _UpLo>
template<typename RhsType, typename DstType>
void LLT<_MatrixType, _UpLo>::_solve_impl(const RhsType& rhs, DstType& dst)
    const
{
 ...
 */
template<typename Derived>
inline const LLT<typename MatrixBase<Derived>::PlainObject> MatrixBase<
    Derived>::llt() const
{
