...
{
    typedef const MatrixType* ConstMatrixPtr;
...
protected:
    ...
    void factorize(const MatrixType& a)
    {
        ...
        if (m_P.size() == 0 && (int(UpLo) & int(Upper)) == Upper) {
            ...
            // without any copy
            internal::
                simplicial_cholesky_grab_input<CholMatrixType, MatrixType>::run(
                    a, pmat, tmp);
        }
        else {
...
struct traits<SimplicialLLT<_MatrixType, _UpLo, _Ordering>>
{
    ...
    typedef TriangularView<
        const typename CholMatrixType::AdjointReturnType,
        Eigen::Upper>
        MatrixU;
...
public:
    ...
    {
        eigen_assert(
            Base::m_factorizationIsOk && "Simplicial Cholesky not factorized");
        return Base::m_diag;
    }
