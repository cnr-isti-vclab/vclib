...
template<class Derived>
template<class OtherDerived>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Derived& QuaternionBase<
...
template<class Derived>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE QuaternionBase<Derived>& QuaternionBase<
    Derived>::operator=(const QuaternionBase<Derived>& other)
{
    coeffs() = other.coeffs();
    return derived();
}
    ...
    const MatrixBase<MatrixDerived>& xpr)
{
    EIGEN_STATIC_ASSERT(
        ...
        YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
    internal::quaternionbase_assign_impl<MatrixDerived>::run(
        *this, xpr.derived());
    return derived();
}
...
struct quaternionbase_assign_impl<Other, 3, 3>
{
        ...
        const Other&             a_mat)
    {
        ...
        if (t > Scalar(0)) {
            ...
            t = sqrt(
                mat.coeff(i, i) - mat.coeff(j, j) - mat.coeff(k, k) +
                Scalar(1.0));
