...
public:
    ...
    {
        return m_data
            [StorageOrder == RowMajor ? j + i * m_stride : i + j * m_stride];
    }
...
public:
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketType loadPacket(Index i) const
    {
        return pgather<Scalar, PacketType>(
            m_data + i * m_incr.value(), m_incr.value());
    }
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void storePacket(
        ...
        const PacketType& p) const
    {
        pscatter<Scalar, PacketType>(
            m_data + i * m_incr.value(), p, m_incr.value());
    }
...
{
    return combine_scalar_factors_impl<ResScalar, Lhs, Rhs>::run(
        alpha, lhs, rhs);
}
