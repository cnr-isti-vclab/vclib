...
EIGEN_DEVICE_FUNC inline IndexDest convert_index(const IndexSrc& idx)
{
    ...
    eigen_internal_assert(
        idx <= NumTraits<IndexDest>::highest() &&
...
struct promote_scalar_arg_unsupported<
    ...
    PromotedType,
    false,
    true> : promote_scalar_arg_unsupported<ExprScalar, T, ExprScalar>
{
};
...
struct find_best_packet_helper<Size, PacketType, false>
{
    typedef typename find_best_packet_helper<
        Size,
        typename unpacket_traits<PacketType>::half>::type type;
};
...
struct find_best_packet
{
    typedef typename find_best_packet_helper<
        Size,
        typename packet_traits<T>::type>::type type;
};
    ...
    true> // Try-half
{
    ...
    enum {
        value =
            compute_default_alignment_helper<ArrayBytes, AlignmentBytes / 2>::
                value
    };
};
#else
...
struct cast_return_type
{
    ...
    typedef typename conditional<
        is_same<CurrentScalarType, NewScalarType>::value,
        const XprType&,
        CastType>::type type;
};
