...
pfrexp_generic(const Packet& a, Packet& exponent)
{
    ...
    exponent_offset = pselect(
        is_denormal,
        padd(exponent_offset, normalization_offset),
        exponent_offset);
...
// using "Extended precision modular arithmetic"
    ...
    EIGEN_OPTIMIZATION_BARRIER(x)
    x = pmadd(
        y, pset1<Packet>(1.62865035235881805419921875e-07), x); // = 0x342ee000
    x = pmadd(
        y,
...
template<typename Packet>
EIGEN_STRONG_INLINE void veltkamp_splitting(
    const Packet& x,
    Packet&       x_hi,
    Packet&       x_lo)
{
...
struct ppolevl
{
    ...
    run(const Packet& x, const typename unpacket_traits<Packet>::type coeff[])
    {
        ...
        return pmadd(
            ppolevl<Packet, N - 1>::run(x, coeff), x, pset1<Packet>(coeff[N]));
    }
};
