...
psqrt<Packet16f>(const Packet16f& _x)
{
    ...
    __mmask16 denormal_mask = _mm512_kand(
        _mm512_cmp_ps_mask(
            _x,
            pset1<Packet16f>((std::numeric_limits<float>::min)()),
            _CMP_LT_OQ),
    ...
    return _mm512_mask_blend_ps(
        denormal_mask, pmul(_x, x), _mm512_setzero_ps());
}
...
psqrt<Packet8d>(const Packet8d& _x)
{
    ...
    __mmask16 denormal_mask = _mm512_kand(
        _mm512_cmp_pd_mask(
            _x,
            pset1<Packet8d>((std::numeric_limits<double>::min)()),
            _CMP_LT_OQ),
    ...
    return _mm512_mask_blend_pd(
        denormal_mask, pmul(_x, x), _mm512_setzero_pd());
}
#else
template<>
...
prsqrt<Packet8d>(const Packet8d& _x)
{
    ...
    y_newton = pmul(
        y_newton,
        pmadd(y_newton, pmul(neg_half, y_newton), p8d_one_point_five));
#endif
    // Select the result of the Newton-Raphson step for positive finite
    // arguments. For other arguments, choose the output of the intrinsic. This
    // will return rsqrt(+inf) = 0, rsqrt(x) = NaN if x < 0, and rsqrt(0) =
