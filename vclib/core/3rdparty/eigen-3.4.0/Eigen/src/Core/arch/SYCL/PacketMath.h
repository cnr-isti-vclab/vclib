...
#define SYCL_PLOADT_RO(address_space_target)                                  \
    ...
    {                                                                         \
        ...
        typedef cl::sycl::multi_ptr<                                          \
            scalar,                                                           \
            cl::sycl::access::address_space::address_space_target>            \
             multi_ptr;                                                       \
        auto res = packet_type(                                               \
            static_cast<typename unpacket_traits<packet_type>::type>(0));     \
        res.load(                                                             \
            0, multi_ptr(const_cast<typename multi_ptr::pointer_t>(from)));   \
        return res;                                                           \
    }
...
ploadt_ro(const Eigen::TensorSycl::internal::
              RangeAccess<cl::sycl::access::mode::read_write, T>& from)
{
...
{
    return cl::sycl::fmax(
        cl::sycl::fmax(a.x(), a.y()), cl::sycl::fmax(a.z(), a.w()));
}
...
{
    return cl::sycl::fmin(
        cl::sycl::fmin(a.x(), a.y()), cl::sycl::fmin(a.z(), a.w()));
}
...
template<>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE cl::sycl::cl_float4 pabs<
    cl::sycl::cl_float4>(const cl::sycl::cl_float4& a)
{
    return cl::sycl::cl_float4(
        cl::sycl::fabs(a.x()),
        cl::sycl::fabs(a.y()),
...
template<>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE cl::sycl::cl_double2 pabs<
    cl::sycl::cl_double2>(const cl::sycl::cl_double2& a)
{
...
#define SYCL_POR(TYPE)                                                    \
    template<>                                                            \
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE por<TYPE>(                 \
        const TYPE& a, const TYPE& b)                                     \
    {                                                                     \
        return convert_vector<TYPE>(vector_as_int(a) | vector_as_int(b)); \
    }
