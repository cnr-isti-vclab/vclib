...
#ifdef EIGEN_VECTORIZE_SSE3
    res.v =
        _mm_castpd_ps(_mm_loaddup_pd(reinterpret_cast<const double*>(&from)));
#else
    res.v = _mm_castpd_ps(_mm_load_sd(reinterpret_cast<const double*>(&from)));
    ...
    const Packet2cf&     from)
{
    EIGEN_DEBUG_UNALIGNED_STORE pstoreu(
        &numext::real_ref(*to), Packet4f(from.v));
}
