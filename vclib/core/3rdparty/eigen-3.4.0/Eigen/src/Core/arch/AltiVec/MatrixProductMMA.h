...
EIGEN_ALWAYS_INLINE void pgerMMA(
    __vector_quad*   acc,
    const RhsPacket& a,
    const LhsPacket& b)
{
    ...
    const Packet2d&      b)
{
    if (NegativeAccumulate) {
        __builtin_mma_xvf64gernp(
            acc, (__vector_pair) a, (__vector unsigned char) b);
    }
    else {
...
#define MICRO_MMA_ONE_PEEL                                          \
    if (sizeof(Scalar) == sizeof(float)) {                          \
        MICRO_MMA_UNROLL_TYPE_PEEL(                                 \
            MICRO_MMA_WORK_ONE, MICRO_MMA_LOAD_ONE, RhsPacket);     \
    }                                                               \
    else {                                                          \
        MICRO_MMA_UNROLL_TYPE_PEEL(                                 \
...
#define MICRO_COMPLEX_MMA_TYPE_PEEL(func, func2, type, peel)        \
    if (PEEL_COMPLEX_MMA > peel) {                                  \
        ...
        ploadRhsMMA<Scalar, type>(                                  \
            rhs_ptr_real + (accRows * peel), rhsV##peel);           \
        if (!RhsIsReal) {                                           \
            ploadRhsMMA<Scalar, type>(                              \
                rhs_ptr_imag + (accRows * peel), rhsVi##peel);      \
        }                                                           \
        else {                                                      \
