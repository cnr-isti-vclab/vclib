...
struct dhs_cpack
{
        ...
        Index                 offset)
    {
        ...
        for (; j + vectorSize <= rows; j += vectorSize) {
            ...
            rir +=
                ((PanelMode) ? (vectorSize * (2 * stride - depth)) :
                               vectorDelta);
        }
        ...
        Index                 offset)
    {
        ...
        for (; j + 2 * vectorSize <= cols; j += 2 * vectorSize) {
            ...
            for (; i < depth; i++) {
                ...
                bload<DataMapper, PacketC, Index, 2, 0, ColMajor>(
                    cblock, rhs, i, j);
            ...
            rir +=
                ((PanelMode) ? (2 * vectorSize * (2 * stride - depth)) :
                               vectorDelta);
        }
...
EIGEN_ALWAYS_INLINE void pger_common(
    ...
    const Packet*           rhsVi)
{
    ...
    if (LhsIsReal) {
        ...
        if (!RhsIsReal) {
            pger_common<Packet, ConjugateLhs == ConjugateRhs>(
                accReal, lhsVi, rhsVi);
            pger_common<Packet, ConjugateRhs>(accImag, lhsV, rhsVi);
        }
        else {
    ...
    const Packet&     pAlpha)
{
    ...
    for (; k < depth; k++) {
        Packet rhsV[1];
        rhsV[0] = pset1<Packet>(rhs_ptr[0]);
        pger<1, Scalar, Packet, Index, false>(
...
#define MICRO_ONE_PEEL4                                       \
    MICRO_UNROLL_TYPE_PEEL(                                   \
        4, MICRO_TYPE_PEEL4, MICRO_WORK_ONE, MICRO_LOAD_ONE); \
    rhs_ptr += (accRows * PEEL);
...
#define MICRO_ONE_PEEL1                                       \
    MICRO_UNROLL_TYPE_PEEL(                                   \
        1, MICRO_TYPE_PEEL1, MICRO_WORK_ONE, MICRO_LOAD_ONE); \
...
#define MICRO_STORE_ONE(iter)                                               \
    if (unroll_factor > iter) {                                             \
        ...
        res.template storePacketBlock<Packet, 4>(                           \
            row + iter * accCols, col, acc);                                \
    }
...
#define MICRO_COL_STORE_ONE(iter)                                           \
    if (unroll_factor > iter) {                                             \
        acc.packet[0] =                                                     \
            res.template loadPacket<Packet>(row + iter * accCols, col + 0); \
        bscale<Packet>(acc, accZero##iter, pAlpha);                         \
        res.template storePacketBlock<Packet, 1>(                           \
            row + iter * accCols, col, acc);                                \
    }
    ...
    const Packet&     pMask)
{
    ...
    if ((remaining_depth == depth) && (rows >= accCols)) {
        ...
        for (; k < depth; k++) {
            Packet rhsV[4], rhsVi[4];
            pbroadcast4_old<Packet>(
                rhs_ptr_real, rhsV[0], rhsV[1], rhsV[2], rhsV[3]);
            if (!RhsIsReal)
                pbroadcast4_old<Packet>(
                    rhs_ptr_imag, rhsVi[0], rhsVi[1], rhsVi[2], rhsVi[3]);
            pgerc<
    ...
    Index             offsetB)
{
    ...
    gemm_function = &Eigen::internal::gemmMMA<
        float,
        Index,
        Packet,
        RhsPacket,
    ...
    if (__builtin_cpu_supports("arch_3_1") && __builtin_cpu_supports("mma")) {
        ...
        gemm_function = &Eigen::internal::gemm<
            float,
            Index,
            Packet,
            RhsPacket,
