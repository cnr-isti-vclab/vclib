...
struct round_impl
{
    ...
    {
        EIGEN_STATIC_ASSERT(
            (!NumTraits<Scalar>::IsComplex), NUMERIC_TYPE_MUST_BE_REAL)
...
struct pow_impl
{
    ...
    static EIGEN_DEVICE_FUNC inline result_type run(
        const ScalarX& x,
        const ScalarY& y)
    {
        EIGEN_USING_STD(pow);
...
EIGEN_DEVICE_FUNC typename internal::enable_if<
    (!internal::is_integral<T>::value) && (!NumTraits<T>::IsComplex),
    bool>::type
...
template<>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE double mini(
    const double& x,
    const double& y)
{
    return fmin(x, y);
...
template<>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE long double mini(
    const long double& x,
    const long double& y)
{
...
template<>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE double maxi(
    const double& x,
    const double& y)
{
    return fmax(x, y);
...
template<>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE long double maxi(
    const long double& x,
    const long double& y)
{
#if defined(EIGEN_HIPCC)
...
template<>
EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_DEVICE_FUNC bool sqrt<
    bool>(const bool& x)
{
    return x;
}
...
template<>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE double fmod(
    const double& a,
    const double& b)
{
    return ::fmod(a, b);
