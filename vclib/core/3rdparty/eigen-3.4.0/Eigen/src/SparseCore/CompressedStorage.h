...
public:
    ...
    CompressedStorage& operator=(const CompressedStorage& other)
    {
        ...
        if (other.size() > 0) {
            internal::smart_copy(
                other.m_values, other.m_values + m_size, m_values);
            internal::smart_copy(
                other.m_indices, other.m_indices + m_size, m_indices);
        }
        return *this;
    }
        ...
        const Scalar& defaultValue = Scalar(0))
    {
        ...
        if (id >= m_size || m_indices[id] != key) {
            if (m_allocatedSize < m_size + 1) {
                ...
                internal::smart_copy(
                    m_indices, m_indices + id, newIndices.ptr());
...
protected:
    inline void reallocate(Index size)
    {
        ...
        if (copySize > 0) {
            internal::smart_copy(
                m_values, m_values + copySize, newValues.ptr());
            internal::smart_copy(
                m_indices, m_indices + copySize, newIndices.ptr());
        }
        std::swap(m_values, newValues.ptr());
        std::swap(m_indices, newIndices.ptr());
