...
public:
    ...
    /**
     * @brief Performs an insertion of a new polygon that generates a given
     * number of triangles.
     *
     * The id of the polygon is assumed to be the next available id
     * (i.e. polygonNumber()), and the triangles generated by the polygon
     * are assumed to be the next available triangles in the BiMap
     * (i.e. starting from triangleNumber()).
     *
     * The function assumes that the triangles generated by the polygon are
     * contiguous in the Triangle Mesh.
     *
     * @param[in] numberOfTriangles: the number of triangles generated by the
     * polygon that will be inserted in the BiMap.
     */
    void pushPolygon(uint numberOfTriangles)
    {
        mPolyToTri.push_back(mTriToPoly.size());
        mTriToPoly.reserve(mTriToPoly.size() + numberOfTriangles);
        for (uint i = 0; i < numberOfTriangles; ++i)
            mTriToPoly.push_back(mPolyToTri.size() - 1);
    }
