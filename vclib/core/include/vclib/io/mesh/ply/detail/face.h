    ...
    const std::vector<std::pair<Scalar, Scalar>>& wedges)
{
    ...
    if (!splitFace) {
        ...
        for (uint ff = m.index(f); ff < m.faceCount(); ++ff) {
            // for each vertex/wedge of the triangle
            for (uint i = 0; i < m.face(ff).vertexCount(); ++i) {
    ...
    // loading texture id associated to ALL the wedges
        ...
        if constexpr (HasPerFaceMaterialIndex<MeshType>) {
            if (isPerFaceMaterialIndexAvailable(mesh)) {
                ...
                // n triangles of mesh
                for (uint ff = mesh.index(f); ff < mesh.faceCount(); ++ff) {
    ...
    for (const FaceType& f : mesh.faces()) {
        for (const PlyProperty& p : header.faceProperties()) {
            ...
            if (p.name == ply::vertex_indices) {
                ...
                if constexpr (HasPerFaceWedgeTexCoords<MeshType>) {
                    io::writeProperty(
                        file, f.vertexCount() * 2, p.listSizeType, format);
