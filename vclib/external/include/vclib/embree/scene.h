...
public:
    ...
    /**
     * @brief The type representing a filtered sequence hit along a ray.
     *
     * The tuple contains:
     * - the id of the (polygonal) face that has been hit
     * - the barycentric coordinates of the hit point
     * - the id of the (triangulated) triangle within that face
     * - the parametric distance `t` of the hit along the ray direction
     */
    using FilteredHitResult = std::tuple<uint, Point3f, uint, float>;
     ...
     * and hit filtering.
     *
     * @return A vector of filtered hits ordered by increasing ray parameter
     * `t`. Each element is a @ref FilteredHitResult tuple.
     */
    template<typename ScalarType>
    std::vector<FilteredHitResult> collectFilteredHits(
        const Point3<ScalarType>& origin,
        const Point3<ScalarType>& direction,
        float                     eps) const
    {
        std::vector<FilteredHitResult> filteredHits;
        ...
        for (uint step = 0; step < MAX_RAY_STEPS; ++step) {
            const float originAdvance = (step == 0) ? -eps : rayAdvance;
            const float rayEps        = (step == 0) ? 0.f : eps;

            const Point3<ScalarType> currentOrigin =
                origin + direction * originAdvance;

            RTCRayHit rayhit =
                initRayHitValues(currentOrigin, direction, rayEps);
            ...
            const float localTHit = rayhit.ray.tfar;
            const float tHit      = originAdvance + localTHit;
     ...
     */
    template<typename ScalarType>
    std::vector<FilteredHitResult> collectFilteredHitsNoEPS(
        const Point3<ScalarType>& origin,
