...
public:
     ...
     */
    using HitResult = std::tuple<uint, Point3f, uint, float>;
        ...
        float far = std::numeric_limits<float>::infinity()) const
    {
        if (mScene) {
            ...
            if (rayhit.hit.geomID != RTC_INVALID_GEOMETRY_ID) {
                ...
                return std::make_tuple(fID, barycentricCoords, triID, t);
            }
        }

        return std::make_tuple(UINT_NULL, Point3f(), uint(0), float(0));
    }
                 ...
                 Point3Concept<std::ranges::range_value_t<R2>>
    {
        ...
        if (mScene) {
            ...
            auto computeChunk = [&](uint chunk) {
                ...
                for (std::size_t i = first; i < last; ++i) {
                    ...
                    if (rayHits.hit.geomID[idx] != RTC_INVALID_GEOMETRY_ID) {
                        ...
                        float t = rayHits.ray.tfar[idx];
                        results[i] =
                            std::make_tuple(fID, barycentricCoords, triID, t);
                    }
                    else {
                        results[i] = std::make_tuple(
                            UINT_NULL, Point3f(), uint(0), float(0));
                    }
                }
            };
            ...
            std::fill(
                results.begin(),
                results.end(),
                std::make_tuple(UINT_NULL, Point3f(), uint(0), float(0)));
        }
    ...
    std::vector<HitResult> collectFilteredHits(
        ...
        float                     eps) const
    {
        std::vector<HitResult> filteredHits;
