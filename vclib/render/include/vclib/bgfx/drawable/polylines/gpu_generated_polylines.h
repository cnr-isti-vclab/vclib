/*****************************************************************************
 * VCLib                                                                     *
 * Visual Computing Library                                                  *
 *                                                                           *
 * Copyright(C) 2021-2025                                                    *
 * Visual Computing Lab                                                      *
 * ISTI - Italian National Research Council                                  *
 *                                                                           *
 * All rights reserved.                                                      *
 *                                                                           *
 * This program is free software; you can redistribute it and/or modify      *
 * it under the terms of the Mozilla Public License Version 2.0 as published *
 * by the Mozilla Foundation; either version 2 of the License, or            *
 * (at your option) any later version.                                       *
 *                                                                           *
 * This program is distributed in the hope that it will be useful,           *
 * but WITHOUT ANY WARRANTY; without even the implied warranty of            *
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the              *
 * Mozilla Public License Version 2.0                                        *
 * (https://www.mozilla.org/en-US/MPL/2.0/) for more details.                *
 ****************************************************************************/

#ifndef VCL_BGFX_DRAWABLE_POLYLINES_GPU_GENERATED_POLYLINES_H
#define VCL_BGFX_DRAWABLE_POLYLINES_GPU_GENERATED_POLYLINES_H

#include "polyline_settings.h"

#include <vclib/bgfx/buffers.h>
#include <vclib/bgfx/context.h>
#include <vclib/bgfx/drawable/lines_common/lines.h>

namespace vcl {

class GPUGeneratedPolylines : public Lines<PolylineSettings>
{
    bgfx::ProgramHandle mComputeVertexPH =
        Context::instance()
            .programManager()
            .getComputeProgram<ComputeProgram::POLYLINES>();

    bgfx::ProgramHandle mLinesPH =
        Context::instance()
            .programManager()
            .getProgram<VertFragProgram::POLYLINES>();

    // buffer containing the input vert coords - will be the input of the compute
    // shader
    VertexBuffer mVertCoords;

    // buffer containing the input vert colors - will be the input of the compute
    // shader
    VertexBuffer mVertColors;

    // buffer containing the input vert normals - will be the input of the compute
    // shader
    VertexBuffer mVertNormals;

    // buffer of vertices generated by the compute shader, and used by the
    // vertex/fragment shaders
    VertexBuffer mVertices;

    // buffers of segment and joint indices generated by the compute shader, and
    // used by the vertex/fragment shaders
    IndexBuffer mSegmentIndices;
    IndexBuffer mJointIndices;

    Uniform mComputeData = Uniform("u_numWorksGroups", bgfx::UniformType::Vec4);

public:
    GPUGeneratedPolylines();

    GPUGeneratedPolylines(
        const std::vector<float>& vertCoords,
        const std::vector<uint>&  vertColors,
        const std::vector<float>& vertNormals);

    void swap(GPUGeneratedPolylines& other);

    friend void swap(GPUGeneratedPolylines& a, GPUGeneratedPolylines& b)
    {
        a.swap(b);
    }

    void draw(uint viewId) const;

    void setPoints( 
        const std::vector<float>& vertCoords,
        const std::vector<uint>&  vertColors,
        const std::vector<float>& vertNormals);

private:
    void checkCaps() const
    {
        const bgfx::Caps* caps = bgfx::getCaps();
        const bool computeSupported = bool(caps->supported & BGFX_CAPS_COMPUTE);
        if (!computeSupported) {
            throw std::runtime_error("GPU compute not supported");
        }
    }
    void setCoordsBuffers(const std::vector<float>& vertCoords);

    void setColorsBuffers(const std::vector<uint>& vertColors);

    void setNormalsBuffers(const std::vector<float>& vertNormals);

    void allocateVertexBuffer(uint pointSize);

    void allocateIndexBuffer(uint pointSize);

    void generateVerticesAndIndicesBuffers(uint pointSize);
};

} // namespace vcl

#endif // VCL_BGFX_DRAWABLE_POLYLINES_GPU_GENERATED_POLYLINES_H
