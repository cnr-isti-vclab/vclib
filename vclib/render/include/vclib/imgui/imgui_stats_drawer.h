...
public:
    ...
    void onDraw(uint viewId)
    {
        ...
        static const std::unordered_map<uint16_t, const char*> sVendorIdNames =
            {
        ...
        ImGui::Text(
            "Vendor ID: %s",
            it != sVendorIdNames.end() ? it->second : "Unknown");
        ...
        ImGui::Text(
            "RT memory: %d MB", int(stats->rtMemoryUsed / (1024 * 1024)));
        ...
        ImGui::Text(
            "Submit CPU %0.3f, GPU %0.3f (L: %d) ms",
        ...
        // Update frame times
        std::rotate(
            mFrameTimes.begin(), mFrameTimes.begin() + 1, mFrameTimes.end());
        mFrameTimes.back() = io.Framerate;

        // Display ImGui information
        ImGui::Text(
            "Display size: %g x %g", io.DisplaySize.x, io.DisplaySize.y);
        ImGui::Text(
