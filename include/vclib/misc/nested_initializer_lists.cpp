 ...
 *
 * \code{.cpp}
 * std::list<size_t> list = vcl::NestedInitializerListsProcessor<int, 2>::maxDimensionsLevels({{2,
 * 3, 5, 4}, {2}, {2, 4, 6}});
 * //the list will contain the values 3 and 4.
 * std::list<size_t> list = vcl::NestedInitializerListsProcessor<int, 3>::maxDimensionsLevels( {{{2,
 * 3}, {5, 4}, {6}, {2, 5}}, {{2}, {3, 4, 6}}} );
...
std::list<size_t>
NestedInitializerListsProcessor<T, L>::maxDimensionsLevels(NestedInitializerLists<T, L> values)
{
	std::list<size_t> final;
	bool              first = true;
	for (auto nested : values) {
		std::list<size_t> l =
			NestedInitializerListsProcessor<T, L - 1>::maxDimensionsLevels(nested);
...
/**
 * @brief Applies the lambda function passed as parameter to all the elements of the
 ...
 * NestedInitializerListsProcessor<T, N>::processElements(values, [&iterator](T value) {
 * *(iterator++) = value; }); \endcode
 *
 * @note This function does not take into account the sizes of every list. If some values are
 * missing from the lists they will be skipped and function is not applied.
 * @param values
 ...
 * // push values properly in sizes (example: sizes = NestedInitializerListsProcessor<T,
 * // N>::maxDimensionsLevel)
 *
 * typename std::vector<T>::iterator iterator = v.begin();
 * NestedInitializerListsProcessor<T, N>::processElements(values, [&iterator](T value) {
 *     *(iterator++) = value;
 ...
 * \endcode
 *
 * @note This function takes into account the sizes of every list fits every list in the given
 * sizes. If some values are missing in a list, a zero vale casted to the type T is assed to the
 * function.
 * @param values
 * @param function: a lambda function that takes an argument of type T as input.
 * @param sizes
