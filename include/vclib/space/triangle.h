...
public:
    ...
    ScalarType sideLength(uint i) const { return p[i].dist(p[(i + 1) % 3]); }

    ScalarType sideLength0() const { return p[0].dist(p[1]); }

    ScalarType sideLength1() const { return p[1].dist(p[2]); }

    ScalarType sideLength2() const { return p[2].dist(p[0]); }
