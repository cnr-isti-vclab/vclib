	...
	const Sphere<typename GridType::ScalarType>& s) const
{
	...
	// will use this set only if the value type is not a point -- that is when
	// the value can occupy more than one single cell. We therefore push the
	// results in a set in order to avoid possible duplicates
	IterSet valuesSet;
	...
	typename GridType::ScalarType& dist) const
{
	...
	if (qvv) {
		...
		// where the query value lies (if the cells were empty, we did not found
		// nothing). we now have to expand the search on adjacent cells starting
		// from the center of the query value and expanding until we find a
		// value

		// now we start with the actual search, including all the cells in the
		// actual interval
