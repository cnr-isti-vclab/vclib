...
/**
 * @brief Returns a const reference of the Face at the i-th position in the Face
 ...
 * faceContainerSize(), this function will return a valid Face reference (note
 * that the Face may have been flagged as deleted).
 *
 ...
 *
 * If faceNumber() != faceContainerSize(), it means that there are some Faces
 ...
 * added Face.
 *
 * This means that, if you want to add 5 Faces and this member function returns
 * 4, the added Faces will have id from 4 to id 8 included.
 *
 * If the call of this function will cause a reallocation of the FaceContainer,
 * the function will automatically take care of updating all the Face pointers
 ...
 * avoid unuseful reallocations of the container, saving execution time.
 *
 ...
 *
 * If the call of this function will cause a reallocation of the Face container,
 * the function will automatically take care of updating all the Face pointers
 * contained in the Mesh.
 *
...
/**
 * @brief Compacts the FaceContainer, removing all the Faces marked as deleted.
 * Faces indices will change accordingly. The function will automatically take
 * care of updating all the Face pointers contained in the Mesh.
 */
template<FaceConcept T>
 ...
 * deleted Faces will stay in the Face Container, but will be marked as deleted.
 *
 * Deleted Faces are automatically jumped by the iterators provided by the Face
 * Container.
 *
 ...
 * elements before the Face with the given index.
 *
 ...
 *
 * This is useful if you need to know the indices of the Faces that they would
 * have in a compact container, without considering the deleted ones.
 *
 * @return A vector containing, for each Face index, its index if the container
 * would be compact.
 */
template<FaceConcept T>
...
/**
 * @brief Updates all the indices and pointers of the Faces of this container
 * that are stored in any container of the mesh, according to the mapping stored
 * in the newIndices vector, that tells for each old Face index, the new Face
 * index.
 *
 * This function is useful when delete some Faces, and you want to update the
 * indices/pointers stored in all the containers of the mesh accordingly.
 *
 * E.g. Supposing you deleted a set of Faces, you can give to this function the
 * vector telling, for each one of the old Face indices, the new Face index (or
 * UINT_NULL if you want to leave it unreferenced). This function will update
 * all the pointers stored in the mesh containers accordingly (if they store
 * adjacencies to the Faces).
 *
 * @note This function *does not change the position of the Faces in this
 * container*. It just updates the indices/pointers of the Faces stored in this
 * or other containers.
 *
 * @param[in] newIndices: a vector that tells, for each old Face index, the new
 * Face index. If the old Face must be left as unreferenced (setting `nullptr`
 * to the pointers), the value of the vector must be UINT_NULL.
 */
template<FaceConcept T>
...
/**
 * @brief Returns a small view object that allows to iterate over the Faces of
 * the containers, providing two member functions begin() and end().
 *
 * This member function is very useful when you want to iterate over the Faces
 * using the C++ foreach syntax:
 *
 * @code{.cpp}
 * for (Face& e : m.faces()){
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceAdjacentEdgesEnabled() const
	requires face::HasOptionalAdjacentEdges<T>
{
 ...
 * adjacent edges number will be the same of the vertex number for each face of
 * the container. This is because, for Faces, Adjacent Edges number is tied to
 * the number of vertices.
 */
template<FaceConcept T>
void FaceContainer<T>::enablePerFaceAdjacentEdges()
	requires face::HasOptionalAdjacentEdges<T>
{
 ...
 */
template<FaceConcept T>
void FaceContainer<T>::disablePerFaceAdjacentEdges()
	requires face::HasOptionalAdjacentEdges<T>
{
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceAdjacentFacesEnabled() const
	requires face::HasOptionalAdjacentFaces<T>
{
	return Base::template isOptionalComponentEnabled<
 ...
 *
 * @note If the Face is polygonal (dynamic size, N < 0), when enabled, the
 * adjacent faces number will be the same of the vertex number for each face of
 * the container. This is because, for Faces, Adjacent Faces number is tied to
 * the number of vertices.
 */
template<FaceConcept T>
void FaceContainer<T>::enablePerFaceAdjacentFaces()
	requires face::HasOptionalAdjacentFaces<T>
{
 ...
 */
template<FaceConcept T>
void FaceContainer<T>::disablePerFaceAdjacentFaces()
	requires face::HasOptionalAdjacentFaces<T>
{
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceColorEnabled() const
	requires face::HasOptionalColor<T>
{
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceMarkEnabled() const
	requires face::HasOptionalMark<T>
{
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceNormalEnabled() const
	requires face::HasOptionalNormal<T>
{
 ...
 */
template<FaceConcept T>
void FaceContainer<T>::disablePerFaceNormal()
	requires face::HasOptionalNormal<T>
{
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFacePrincipalCurvatureEnabled() const
	requires face::HasOptionalPrincipalCurvature<T>
{
	return Base::template isOptionalComponentEnabled<
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceQualityEnabled() const
	requires face::HasOptionalQuality<T>
{
 ...
 */
template<FaceConcept T>
void FaceContainer<T>::enablePerFaceQuality()
	requires face::HasOptionalQuality<T>
{
 ...
 */
template<FaceConcept T>
void FaceContainer<T>::disablePerFaceQuality()
	requires face::HasOptionalQuality<T>
{
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceWedgeColorsEnabled() const
	requires face::HasOptionalWedgeColors<T>
{
 ...
 * OptionalWedgeColors Component.
 *
 * @note If the Face is polygonal (dynamic size, N < 0), when enabled, the wedge
 * colors number will be the same of the vertex number for each face of the
 * container. This is because, for Faces, Wedge Colors number is tied to the
 * number of vertices.
 */
template<FaceConcept T>
void FaceContainer<T>::enablePerFaceWedgeColors()
	requires face::HasOptionalWedgeColors<T>
{
 ...
 */
template<FaceConcept T>
void FaceContainer<T>::disablePerFaceWedgeColors()
	requires face::HasOptionalWedgeColors<T>
{
 ...
 */
template<FaceConcept T>
bool FaceContainer<T>::isPerFaceWedgeTexCoordsEnabled() const
	requires face::HasOptionalWedgeTexCoords<T>
{
	return Base::template isOptionalComponentEnabled<
 ...
 *
 * @note If the Face is polygonal (dynamic size, N < 0), when enabled, the wedge
 * texcoord number will be the same of the vertex number for each face of the
 * container. This is because, for Faces, Wedge TexCoords number is tied to the
 * number of vertices.
 */
template<FaceConcept T>
void FaceContainer<T>::enablePerFaceWedgeTexCoords()
	requires face::HasOptionalWedgeTexCoords<T>
{
 ...
 */
template<FaceConcept T>
void FaceContainer<T>::disablePerFaceWedgeTexCoords()
	requires face::HasOptionalWedgeTexCoords<T>
{
 ...
 * any type associated to the Face Element.
 *
 * @note This function is available only if the Face Element has the
 * CustomComponents Component.
 *
...
/**
 * @brief Checks if the custom component of the Face Element having the given
 * name has the same type of the given template argument type of this function.
 *
...
template<FaceConcept T>
template<typename K>
bool FaceContainer<T>::isPerFaceCustomComponentOfType(
...
std::type_index
FaceContainer<T>::perFaceCustomComponentType(const std::string& name) const
	requires face::HasCustomComponents<T>
{
...
FaceContainer<T>::perFaceCustomComponentNamesOfType() const
	requires face::HasCustomComponents<T>
{
...
template<typename K>
ConstCustomComponentVectorHandle<K>
FaceContainer<T>::perFaceCustomComponentVectorHandle(
	const std::string& name) const requires face::HasCustomComponents<T>
{
	return Base::template customComponentVectorHandle<K>(name);
}
...
/**
 * @brief This function manages the case where we try to import into a TriMesh a
 * PolyMesh Faces have been already imported, but without vertex pointers and
 * other components that depend on the number of vertices (e.g. wedges)
 */
...
void FaceContainer<F>::manageImportTriFromPoly(const OthMesh& m)
{
	if constexpr (HasFaceContainer<OthMesh>) {
		...
		static_assert(
			...
			"Cannot import from that type of Mesh. Don't know how to convert "
			"faces.");
