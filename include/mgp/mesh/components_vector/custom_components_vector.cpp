...
template<typename T>
void CustomComponentsVector<T, IfHasCustomComp<T>>::resize(unsigned int size)
{
	for (auto& p : map) {
...
CustomComponentsVector<T, IfHasCustomComp<T>>::componentVector(const std::string& attrName) const
{
...
template<typename AttrType>
std::vector<std::any>&
CustomComponentsVector<T, IfHasCustomComp<T>>::componentVector(const std::string& attrName)
{
