    ...
    while(true) {
        ...
        for (uint i = 0; const auto& [faceId, barCoords, tId] : hits) {
            ...
            if (faceId != UINT_NULL) { // should never be UINT_NULL here
                ...
                if (rayIdx < origins.size()) {
                    origins[rayIdx] = hitPos;
                }

            }

            else {
                if (rayIdx < origins.size() && rayIdx < directions.size()) {
                    origins.erase(origins.begin() + rayIdx);
                    directions.erase(directions.begin() + rayIdx);
                }
            }

        }

        if (origins.empty() || directions.empty()) {
            std::cout << "No more rays to test. Exiting.\n";
            break;
        }
    }
    return 0;
