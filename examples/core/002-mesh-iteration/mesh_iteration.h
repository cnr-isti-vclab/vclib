...
void meshIteration()
{
    ...
    for (int i = 0; i < 3 && it != mesh.vertexEnd(); ++it, ++i) {
        std::cout << "  Vertex " << it->index() << ": " << it->position()
                  << std::endl;
    }
    ...
    for (int count = 0; const auto& position :
                        mesh | vcl::views::vertices | vcl::views::positions) {
        if (count >= 3)
            break;
    ...
    for (int count = 0; const auto& face : mesh | vcl::views::faces) {
        ...
        for (const auto& position :
             face | vcl::views::vertices | vcl::views::positions) {
    ...
    for (int count = 0; const auto& face : mesh | vcl::views::faces) {
        ...
        for (const auto* adjFace :
             face | vcl::views::adjFaces | vcl::views::notNull) {
            std::cout << adjFace->index() << " ";
        }
        std::cout << std::endl;
        count++;
    ...
    std::cout << "Mesh bounding box: [" << minPoint << "] to [" << maxPoint
              << "]" << std::endl;
    ...
    std::cout << "- Range-based for loops are generally the most readable"
              << std::endl;
    std::cout << "- Index-based access can be faster for random access patterns"
              << std::endl;
    std::cout << "- Views provide composable iteration patterns" << std::endl;
}

#endif // MESH_ITERATION_H
