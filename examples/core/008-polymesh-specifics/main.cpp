...
int main()
{
    ...
    std::cout << "  " << triMeshLoaded.faceNumber() << " faces with 3 vertices each\n\n";
    ...
    std::cout << "After adding 1st vertex: " << squareFace.vertexNumber() << " vertices\n";

    squareFace.pushVertex(1u);
    squareFace.pushVertex(3u);
    squareFace.pushVertex(2u);
    std::cout << "After adding all vertices: " << squareFace.vertexNumber() << " vertices\n";

    // Clear and reset vertices
    squareFace.clearVertices();
    std::cout << "After clearing: " << squareFace.vertexNumber() << " vertices\n";

    // Set multiple vertices at once
    squareFace.setVertices(0u, 1u, 3u, 2u);
    std::cout << "After setVertices: " << squareFace.vertexNumber() << " vertices\n";
    ...
    if (polyMeshLoaded.faceNumber() > 0) {
        ...
        for (int i = 0; i < face.vertexNumber(); ++i) {
            const auto* adjFace = face.adjFace(i);
            std::cout << "  Edge " << i << " (v" << face.vertexIndex(i)
                      << "-v" << face.vertexIndexMod(i + 1)
                      << "): ";
            if (adjFace != nullptr) {
    ...
    size_t triMeshFaceMemory = triMeshLoaded.faceNumber() * 3; // 3 vertex indices per face
    ...
    std::cout << "   - clearVertices() and setVertices() for face manipulation\n\n";

    std::cout << "2. TRIMESH VS POLYMESH:\n";
    std::cout << "   - TriMesh: fixed 3 vertices per face, optimized for triangles\n";
    std::cout << "   - PolyMesh: variable vertices per face, preserves original geometry\n";
