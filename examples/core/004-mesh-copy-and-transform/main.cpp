...
int main()
{
    ...
    auto bb_before = transformMesh.boundingBox();
    std::cout << "   Bounding box before: min(" << bb_before.min().x() 
              << ", " << bb_before.min().y() << ", " << bb_before.min().z() 
              << ") max(" << bb_before.max().x() << ", " << bb_before.max().y() 
              << ", " << bb_before.max().z() << ")\n";
    ...
    auto bb_after = transformMesh.boundingBox();
    std::cout << "   Bounding box after translation by (" << translation.x() 
              << ", " << translation.y() << ", " << translation.z() << "):\n";
    std::cout << "   min(" << bb_after.min().x() << ", " << bb_after.min().y() 
              << ", " << bb_after.min().z() << ") max(" << bb_after.max().x() 
              << ", " << bb_after.max().y() << ", " << bb_after.max().z() << ")\n";
    ...
    auto bb_scale_before = scaleMesh.boundingBox();
    double diagonal_before = (bb_scale_before.max() - bb_scale_before.min()).norm();
    std::cout << "   Bounding box diagonal before scaling: " 
              << diagonal_before << "\n";
    ...
    auto bb_scale_after = scaleMesh.boundingBox();
    double diagonal_after = (bb_scale_after.max() - bb_scale_after.min()).norm();
    std::cout << "   Diagonal after scaling " << scaleFactor << "x: " 
              << diagonal_after << "\n";
    std::cout << "   Diagonal ratio: " << diagonal_after / diagonal_before 
              << " (should be ~" << scaleFactor << ")\n";
    
    // Non-uniform scaling
    std::cout << "\n   Non-uniform scaling:\n";
    vcl::TriMesh nonUniformMesh = vcl::createCube<vcl::TriMesh>();
    vcl::Point3d scaleFactors(1.0, 2.0, 0.5);  // x unchanged, y doubled, z halved
    vcl::scale(nonUniformMesh, scaleFactors);
    vcl::updateBoundingBox(nonUniformMesh);
    
    auto bb_non_uniform = nonUniformMesh.boundingBox();
    vcl::Point3d size = bb_non_uniform.max() - bb_non_uniform.min();
    std::cout << "   Dimensions after non-uniform scaling (" 
    ...
    std::cout << "   Rotation by " << (angleRadians * 180.0 / M_PI) 
              << " degrees around axis (" << axis.x() << ", " << axis.y() << ", " << axis.z() << ")\n";
    
    vcl::rotate(rotateMesh, axis, angleRadians);
    
    auto bb_rotated = rotateMesh.boundingBox();
    std::cout << "   Bounding box after rotation: dimensions (" 
              << (bb_rotated.max().x() - bb_rotated.min().x()) << ", "
              << (bb_rotated.max().y() - bb_rotated.min().y()) << ", "
              << (bb_rotated.max().z() - bb_rotated.min().z()) << ")\n";
    ...
    transformMatrix.block<3,3>(0,0) *= scale_factor;
    
    std::cout << "Transformation matrix:\n" << transformMatrix << "\n\n";
    
    // Apply the transformation matrix
    auto bb_matrix_before = matrixMesh.boundingBox();
    std::cout << "Before transformation: center = (" 
              << bb_matrix_before.center().x() << ", " << bb_matrix_before.center().y() 
              << ", " << bb_matrix_before.center().z() << ")\n";
    
    vcl::applyTransformMatrix(matrixMesh, transformMatrix);
    vcl::updateBoundingBox(matrixMesh);
    
    auto bb_matrix_after = matrixMesh.boundingBox();
    std::cout << "After transformation: center = (" 
              << bb_matrix_after.center().x() << ", " << bb_matrix_after.center().y() 
              << ", " << bb_matrix_after.center().z() << ")\n";
    ...
    vcl::TriMesh sphere1 = vcl::createSphereIcosahedron<vcl::TriMesh>(
        vcl::Sphere<vcl::TriMesh::ScalarType>(vcl::Point3<vcl::TriMesh::ScalarType>(0,0,0), 1.0), 3);
    vcl::translate(sphere1, vcl::Point3d(3.0, 0.0, 0.0));  // Move the sphere
    
    vcl::TriMesh sphere2 = vcl::createSphereUV<vcl::TriMesh>(
        vcl::Sphere<vcl::TriMesh::ScalarType>(vcl::Point3<vcl::TriMesh::ScalarType>(0,0,0), 1.0), 16, 16);
    vcl::translate(sphere2, vcl::Point3d(-3.0, 0.0, 0.0));  // Move the other sphere
    vcl::scale(sphere2, 0.8);  // Reduce the second sphere
    
    vcl::TriMesh tetrahedron = vcl::createTetrahedron<vcl::TriMesh>();
    vcl::translate(tetrahedron, vcl::Point3d(0.0, 3.0, 0.0));  // Move the tetrahedron
    ...
    auto bb_combined = combinedMesh.boundingBox();
    std::cout << "Bounding box of combined mesh:\n";
    std::cout << "Min: (" << bb_combined.min().x() << ", "
              << bb_combined.min().y() << ", " << bb_combined.min().z() << ")\n";
    std::cout << "Max: (" << bb_combined.max().x() << ", "
              << bb_combined.max().y() << ", " << bb_combined.max().z() << ")\n";
    ...
    try {
        ...
        vcl::save(transformMesh, resultsPath + "/004_transformed_icosahedron.ply");
        std::cout << "Saved: 004_transformed_icosahedron.ply (translated icosahedron)\n";
        ...
        std::cout << "Saved: 004_matrix_transformed.ply (matrix transformation)\n";
