...
auto meshCustomComponents()
{
    ...
    for (auto& vertex : mesh.vertices()) {
        ...
        bool isBoundary =
            (vertex.index() % 10 == 0); // Simplified boundary detection
    ...
    std::cout << "Marked " << boundaryCount
              << " vertices as boundary candidates\n";
    ...
    std::cout << "  Model name: "
              << mesh.customComponent<std::string>("modelName") << "\n";
    std::cout << "  Processing version: "
              << mesh.customComponent<int>("processingVersion") << "\n";
    std::cout << "  Mesh quality: "
              << mesh.customComponent<double>("meshQuality") << "\n";
    ...
    mesh.addCustomComponent<vcl::Point3d>(
    ...
    std::cout << "  Total surface area: "
              << mesh.customComponent<double>("totalSurfaceArea") << "\n";
    std::cout << "  Bounding box size: "
              << mesh.customComponent<vcl::Point3d>("boundingBoxSize") << "\n";
    std::cout << "  Mesh centroid: "
              << mesh.customComponent<vcl::Point3d>("meshCentroid") << "\n\n";
    ...
    for (const auto& vertex : mesh.vertices()) {
        if (vertex.customComponent<int>("processingFlag") == 1) {
            ...
            if (adjCount > 0) {
                ...
                newPositions[vertex.index()] =
                    0.7 * vertex.position() + 0.3 * avgPos;
            }
        }
    }
    ...
    std::cout
        << "   - addPerVertexCustomComponent<T>(name): add typed component\n";
    std::cout
        << "   - vertex.customComponent<T>(name): access component data\n";
    std::cout
        << "   - perVertexCustomComponentVectorHandle<T>(): bulk access\n\n";

    std::cout << "2. FACE CUSTOM COMPONENTS:\n";
    std::cout
        << "   - addPerFaceCustomComponent<T>(name): add typed component\n";
    std::cout << "   - face.customComponent<T>(name): access component data\n";
    std::cout
        << "   - Support for various types: int, double, string, Point3d\n\n";
    ...
    std::cout
        << "   - isPerVertexCustomComponentOfType<T>(name): type checking\n";
    std::cout
        << "   - deletePerVertexCustomComponent(name): remove component\n\n";
    ...
    return std::make_tuple(mesh);
}

#endif // MESH_CUSTOM_COMPONENTS_H
