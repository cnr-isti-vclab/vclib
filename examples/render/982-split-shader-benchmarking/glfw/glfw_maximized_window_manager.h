...
#ifndef VCL_GLFW_MAXIMIZED_WINDOW_MANAGER_H
#define VCL_GLFW_MAXIMIZED_WINDOW_MANAGER_H
...
// Yes, this is a copy and paste of vclib/glfw/window_manager.h with just a
// singular function call added. Extending the class would be about the same
// since DerivedRenderApp::WM only adds the derived class as a friend, and
// therefore I would've had to redefine all the methods anyways
template<typename DerivedRenderApp>
class MaximizedWindowManager
{
    std::string mTitle;
...
public:
     ...
     * initialize the base class (if any). In the glfw::MaximizedWindowManager,
     * the parent class is void and it is not managed.
     */
    ...
    MaximizedWindowManager(ParentType* parent = nullptr) :
            MaximizedWindowManager("", 1024, 768, parent)
    {
    }

    MaximizedWindowManager(
    ...
    virtual ~MaximizedWindowManager() = default;
    ...
    void setCallbacks()
    {
        ...
        glfwSetFramebufferSizeCallback(
            mWindow, [](GLFWwindow* window, int width, int height) {
                auto* self = static_cast<MaximizedWindowManager*>(
