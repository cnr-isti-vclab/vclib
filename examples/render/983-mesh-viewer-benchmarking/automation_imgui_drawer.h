...
class AutomationImguiDrawer : public vcl::PlainDrawer<DerivedDrawer>
{
    ...
    using NameActionPair =
        std::pair<std::string, vcl::AbstractAutomationAction*>;
...
public:
    ...
    size_t addAutomation(
        ...
        vcl::AbstractAutomationAction* action)
    {
        automations.push_back(
            std::pair<NameActionPair::first_type, NameActionPair::second_type>(
                name, action));
